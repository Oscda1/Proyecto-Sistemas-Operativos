...
void list_commands(char *commands[], int *num_commands) {
  DIR *dir;
  struct dirent *entry;
...
void show_man_page(char *command) {
    endwin(); // Finalizar ncurses antes de ejecutar el manual
    if (fork() == 0) {
    ...
    exit(EXIT_SUCCESS);     // Terminar el programa después de salir del manual
}
...
int main() {
  ...
  int num_commands = 0;
  list_commands(commands, &num_commands);
  ...
  while (running) {
    ...
    case 27:       // Escape key to quit
      ...
      break;
    case KEY_BACKSPACE: // Backspace key, borrar el último carácter de la
                        // búsqueda
